						...
						if(target.split(' ').length > 1){
							...
							for(var i in target.split(' ')){
								...
								if(target.split(' ')[i].indexOf('--') !=-1 && target.split(' ')[i].indexOf('__') !=-1){
									...
									editor.find('&__'+target.split(' ')[i].split('__')[target.split(' ')[i].split('__').length-1])
								}
							...
							if (target.indexOf('--') !=-1){
								...
								var tArr = target.split('__');
								editor.find('&__'+tArr[tArr.length-1]);
							}
...
// переменная permission содержит результат запроса
    ...
    var deploy = function(){
        ...
        $('#deploy').on('mousedown', function () {
            ...
            $.ajax({
                url: "http://localhost:8181?action=deploy"
                , success: function (res) {
                    alert('DEPLOYED');
                }
            });
...
$('span#openData').on('mousedown',function () {
	$('#editorData').toggle()
})
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var quantServerCommand = function(triggerStr,actionType,argsObj,callback,getDataCallback=false,method = 'GET'){
	
	
		
		
	$(triggerStr).on(actionType,function(){
		
		if(typeof getDataCallback ==='function') argsObj.data =	getDataCallback();
